...
test('Test 4: maps true, babel false', done => {
    const Data = getDataObject();
    Data.files = [];
    Data.addFile('/resources/src/js/main2.js', '../src/js/main2.js', 'var test = function(x) {return x + 5;}; test();');
    Data.setOption('maps', true);
...
test('Test 7: custom babel option', done => {
    const Data = getDataObject();
    Data.files = [];
    Data.addFile('/resources/src/js/main2.js', '../src/js/main2.js', 'const test = (x) => {return x + 5;}; test();');
    Data.setOption('js', {babel: {presets: []}, uglifyjs: false});
    new JsController().compile(Data).then((Data) => {
        expect(Data.getCompiled().getCode(true)).toEqual(fs.readFileSync(`${dir}/data/js/test7-expected.js`).toString());
        done();
    });
});

test('Test 8: custom uglifyjs option', done => {
    const Data = getDataObject();
    Data.files = [];
    Data.addFile('/resources/src/js/main2.js', '../src/js/main2.js', 'const test = (x) => {return x + 5;}; test();');
    Data.setOption('js', {uglifyjs: {mangle: false}});
    new JsController().compile(Data).then((Data) => {
        expect(Data.getCompiled().getCode(true)).toEqual(fs.readFileSync(`${dir}/data/js/test8-expected.js`).toString());
        done();
    });
});